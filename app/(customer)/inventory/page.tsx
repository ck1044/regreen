...
import { formatInternalApiUrl, INVENTORY_ROUTES, TodayInventoryItem } from '@/app/api/routes';
  ...
  useEffect(() => {
    const fetchInventory = async () => {
      ...
      try {
        ...
        const response = await fetch(formatInternalApiUrl(INVENTORY_ROUTES.TODAY), {
          method: 'GET',
          headers,
          cache: 'no-store'
        });
        ...
        const inventoryData = await response.json() as TodayInventoryItem[];
        ...
        const transformedProducts = inventoryData.map(item => ({
          ...
          shopName: item.store.name || '상점 정보 없음',
          shopId: '1', // API에서 이 정보를 제공하지 않는 경우 기본값 사용
          price: item.inventory.price,
          quantity: 1,  // 수량 정보는 API에서 제공하지 않음
          ...
          category: item.store.category || '기타'
        }));
