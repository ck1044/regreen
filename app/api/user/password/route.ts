...
export async function PATCH(request: NextRequest) {
  try {
    ...
    // API 엔드포인트 구성
    const apiEndpoint = formatExternalApiUrl(USER_ROUTES.UPDATE_PASSWORD);
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { message: responseData.message || '비밀번호 변경 중 오류가 발생했습니다.' },
        { status: response.status }
      );
    }
    
    // 성공 응답 반환
    return NextResponse.json(responseData || { message: '비밀번호가 성공적으로 변경되었습니다.' });
